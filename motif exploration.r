...
for(i in 1:nsp){
# first, subset two species competing with a third
nei<-sum(matp[i,])
idnei<-as.numeric(which(matp[i,]==1))
...
random_motif1=c()
for (x in seq_len(999L)) {
ignew=graph_from_edgelist(as.matrix(rn1[[x]][,-3]) , directed = FALSE) #elist is 0.45edge
...
#overrepresentation of network motifs could be calculated as described by M&M using EXCEL
